...
fn test_cell_map_num_inputs_outputs() {
    ...
    assert_eq!(num_inputs, 2 + 0 + 4 + 0, "Should count all inputs");
    assert_eq!(num_outputs, 0 + 3 + 4 + 0, "Should count all outputs");
}
...
fn test_genome_accessors() {
    ...
    assert!(cells.len() > 0, "Should have cells");

    let hidden = genome.hidden_neurons();
    assert!(hidden.len() > 0, "Should have hidden neurons");
}
