...
use crate::{Cell, Drawable, Log, LogType, On, Organism};
use bevy::{
    ecs::system::Resource,
    math::{I64Vec2, Vec3},
    prelude::default,
...
impl LEWorld {
    ...
    #[cfg(feature = "log")]
    pub fn push_evt(&mut self, actor: Actor, evt: LogType, on: On) {
        self.events.push(Log::new(self.lifetime, actor, evt, on))
    }
                                ...
                                match map.produce_food_around(location) {
                                    ...
                                    Err(_e) => {
                                        ...
                                        events.push(Log::new(
                                            self.lifetime,
                                            organism.read().unwrap().actor(),
                                            LogType::FailProduced(_e.to_string()),
                                            On::Around(location),
                                        ));
                                    }
                                }
                                ...
                                match map.move_organism(&organism, location) {
                                    ...
                                    Err(_e) => {
                                        ...
                                        events.push(Log::new(
                                            self.lifetime,
                                            organism.read().unwrap().actor(),
                                            LogType::FailMoved(_e.to_string()),
                                            On::To(location),
                                        ));
                                    }
                                }
                                ...
                                let mut killed = if cfg!(feature = "log") {
                                    ...
                                    for error in errors {
                                        events.push(Log::new(
                                            self.lifetime,
                                            o.actor(),
                                            LogType::FailKilled(error.to_string()),
                                            On::None,
                                        ));
                                    }
                                    killed
                            ...
                            OrganismRequest::Starve => match map.kill_organism(&organism) {
                                ...
                                Err(_e) => {
                                    ...
                                    events.push(Log::new(
                                        self.lifetime,
                                        Actor::Map,
                                        LogType::FailStarved(_e.to_string()),
                                        On::Actor(organism.read().unwrap().actor()),
                                    ));
                                ...
                                if let Some(max_pop) = self.settings.max_organisms {
                                    if self.organisms.len() >= max_pop {
                                        ...
                                        events.push(Log::new(
                                            self.lifetime,
                                            o.actor(),
                                            LogType::FailReproduced("max pop exceeded".to_string()),
                                            On::None,
                                        ));
                                        continue;
                                    }
                                    ...
                                    Err(_e) => {
                                        ...
                                        */
                                        ...
                                        events.push(Log::new(
                                            self.lifetime,
                                            organism.read().unwrap().actor(),
                                            LogType::FailReproduced(_e.to_string()),
                                            On::None,
                                        ));
                                    }
                                }
                ...
                self.organisms.clone().into_iter().fold(
                    ...
                    |(mut dead_organisms, mut alive_organisms, mut _events), organism| {
                        ...
                        if dead {
                            ...
                            #[cfg(feature = "log")]
                            _events.push(Log::new(
                                self.lifetime,
                                Actor::Map,
                                LogType::MovedToGraveyard,
                                On::Actor(organism.read().unwrap().actor()),
                            ));
                        } else {
